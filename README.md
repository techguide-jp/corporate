...
- 現在の `/redirects?event=...` の遷移先を変えるとき: [`techguide/src/routes/redirects/+server.ts`](./techguide/src/routes/redirects/+server.ts)
...
- `/redirects` の現行実装: [techguide/src/routes/redirects/+server.ts](./techguide/src/routes/redirects/+server.ts)
...
`https://techguide.jp/redirects?event=<keyword>` は、現在 [techguide/src/routes/redirects/+server.ts](./techguide/src/routes/redirects/+server.ts) の `eventRedirectDestinations` マップで制御しています。

例:

- `https://techguide.jp/redirects?event=20230522_talk_seminar`

この導線を変更する場合は、`techguide/src/routes/redirects/+server.ts` を更新してください。
...
- `/redirects?event=...` は `techguide/src/routes/redirects/+server.ts` を正とする
